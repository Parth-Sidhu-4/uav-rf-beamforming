...
content = re.sub(r'data = np\.load\(\"dataset_3d_pilot_masks\.npz\"\).*?N_POINTS = len\(headings_full\)', '''data = np.load(\"dataset_shadow_100k_polar_32el.npz\")
    inputs = data['inputs']
    ...
    N_POINTS = len(headings_full)''', content, flags=re.DOTALL)
...
with open('stage18s_train_100k_physics_fixed.py', 'w') as f:
    f.write(content)
