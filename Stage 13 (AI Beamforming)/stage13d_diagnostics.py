...
def main():
    ...
    if os.path.exists(log_path):
        ...
        if epochs:
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig("diag0_loss_curve.png")
            print("Saved diag0_loss_curve.png")
    ...
    # AI Predictions
    with torch.no_grad():
        preds = model(torch.tensor(jam_bodies, dtype=torch.float32))
        mag_preds = torch.sigmoid(preds[:, 0::2]).numpy()
        phase_preds = preds[:, 1::2].numpy()
    g_ai_all = mag_preds * np.exp(1j * phase_preds)
    ...
    fig.tight_layout()
    plt.grid(True)
    plt.savefig("diag1_mag_phase_error.png")
    print("Saved diag1_mag_phase_error.png")
    ...
    plt.grid(True)
    plt.savefig("diag2_null_error.png")
    print("Saved diag2_null_error.png")
    ...
    plt.grid(True)
    plt.savefig("diag3_noise_sensitivity.png")
    print("Saved diag3_noise_sensitivity.png")
    ...
    plt.grid(True)
    plt.savefig("diag4_dl_sweep.png")
    print("Saved diag4_dl_sweep.png")
