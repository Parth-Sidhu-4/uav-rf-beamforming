...
def main():
    ...
    if os.path.exists(log_path):
        ...
        if epochs:
            ...
            plt.savefig("diag0_loss_curve_sinr.png")
            print("Saved diag0_loss_curve_sinr.png")
    ...
    model = ShadowNet()
    try:
        model.load_state_dict(torch.load("shadow_net_sinr.pt"))
    except Exception:
        print("Model shadow_net_sinr.pt not found. Ensure training is complete.")
        return
    model.eval()
    ...
    fig.tight_layout()
    plt.grid(True)
    plt.savefig("diag1_mag_phase_error_sinr.png")
    print("Saved diag1_mag_phase_error_sinr.png")
    ...
    plt.title("Diagnostic 2: Angular Null-Placement Error (Phase B)")
    plt.xlabel("Jammer Azimuth (deg)")
    plt.ylabel("Null Displacement from True Jammer (deg)")
    plt.grid(True)
    plt.savefig("diag2_null_error_sinr.png")
    print("Saved diag2_null_error_sinr.png")
    ...
    plt.title("Diagnostic 3: MVDR Sensitivity to Complex Gaussian Noise (Phase B)")
    plt.xlabel("Noise Std Dev (σ) on Exact Mask")
    plt.ylabel("Average SINR (dB)")
    plt.grid(True)
    plt.savefig("diag3_noise_sensitivity_sinr.png")
    print("Saved diag3_noise_sensitivity_sinr.png")
    ...
    plt.title("Diagnostic 4: Empirical Diagonal Loading Optimization (Phase B)")
    plt.xlabel("Diagonal Loading Factor (Fraction of Trace)")
    plt.ylabel("Worst-Case SINR over Azimuth Sweep (dB)")
    plt.grid(True)
    plt.savefig("diag4_dl_sweep_sinr.png")
    print("Saved diag4_dl_sweep_sinr.png")
