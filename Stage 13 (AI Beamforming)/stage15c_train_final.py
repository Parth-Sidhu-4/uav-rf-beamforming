...
MSE_WEIGHT = 5.0  # Strong MSE to ensure tethering to reality
...
class FinalLoss(nn.Module):
    ...
    def forward(self, cart_inputs, g_exact_polar, model):
        ...
        # Continuous Complex MSE Tether
        phases_geo_exact = K * (cart_inputs @ self.pos_body.T)
        
        phase_dev_exact = phase_exact - phases_geo_exact
        g_dev_exact = mag_exact * torch.exp(1j * phase_dev_exact)
        
        g_ai = mag_ai * torch.exp(1j * phase_pert)
        
        loss_mse = torch.mean(torch.abs(g_ai - g_dev_exact)**2)

        # SIREN/SM-MVDR Leakage
        geo_sp = torch.exp(1j * phases_geo_exact)
        v_jam_ai = geo_sp * g_ai
...
def train_model(model, criterion, train_loader, val_loader, epochs, save_path=None):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 10 == 0 or epoch == 1:
            print(f"Epoch {epoch:03d}/{epochs} | Tot: {train_loss:.4f} | MSE: {train_mse:.4f} | Null: {train_null:.4f}")
...
def main():
    ...
    print(f"--- Running Final Robust Training ---")
    model = RobustNet()
    criterion = FinalLoss(pos_body, v_sig)
    
    train_model(model, criterion, train_loader, val_loader, epochs=EPOCHS, save_path="shadow_net_final.pt")
    print("Training complete. Saved to shadow_net_final.pt")
