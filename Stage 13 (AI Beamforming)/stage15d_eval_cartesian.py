...
from stage15d_train_cartesian import CartesianShadowNet
...
def main():
    print("Evaluating Phase E Cartesian Robust Architecture (shadow_net_cartesian.pt)")
    ...
    model = CartesianShadowNet().to(device)
    model.load_state_dict(torch.load("shadow_net_cartesian.pt", map_location=device))
    model.eval()
    ...
    with torch.no_grad():
        ...
        v_jam_ai_all = (geo_sp * g_bounded).numpy()

    # Evaluation uses DL_ALPHA = 0 for true null synthesis!
    DL_ALPHA = 0.0
