...
class CartesianShadowNet32(nn.Module):
    def __init__(self):
        super().__init__()
...
class CartesianRobustLoss32(nn.Module):
    ...
    def forward(self, cart_inputs, g_exact_polar, model):
        ...
        B = cart_inputs.shape[0]
        
        # Randomize v_sig to prevent overfitting
        random_az = torch.rand(B, device=device) * 2 * torch.pi
        random_el = (torch.rand(B, device=device) - 0.5) * torch.pi
        sig_world = torch.stack([
            torch.cos(random_el) * torch.cos(random_az),
            torch.cos(random_el) * torch.sin(random_az),
            torch.sin(random_el)
        ], dim=1).to(torch.float32)
        
        phases_sig = K * (sig_world @ self.pos_body.T)
        v_sig_batch = torch.exp(1j * phases_sig).to(torch.complex64)
        ...
        c_B = NOISE_POW + DL_ALPHA * (NOISE_POW + JAM_POW * norm_v2 / 32.0)
        v_H_vsig = torch.sum(torch.conj(v_jam_ai) * v_sig_batch, dim=1)
        term2_scalar = JAM_POW / (c_B * (c_B + JAM_POW * norm_v2))
        u = (1.0 / c_B.unsqueeze(1)) * v_sig_batch - (term2_scalar * v_H_vsig).unsqueeze(1) * v_jam_ai
        vsig_H_u = torch.sum(torch.conj(v_sig_batch) * u, dim=1)
        w = u / vsig_H_u.unsqueeze(1)

        # Exact Null Leakage (No Jitter)
        # We test w directly against the exact true complex steering vector!
        v_jam_exact = g_exact_complex * geo_sp
        leakage = torch.abs(torch.sum(torch.conj(w) * v_jam_exact, dim=1))**2
        loss_null = torch.mean(torch.log10(leakage + 1e-12))
...
def main():
    dataset_path = "dataset_shadow_100k_polar_32el.npz"
    ...
    print(f"--- Running Cartesian Robust Training (32 Elements) ---")
    model = CartesianShadowNet32()
    criterion = CartesianRobustLoss32(pos_body, v_sig)
    
    train_model(model, criterion, train_loader, val_loader, epochs=EPOCHS, save_path="shadow_net_cartesian_32.pt")
    print("Training complete. Saved to shadow_net_cartesian_32.pt")
