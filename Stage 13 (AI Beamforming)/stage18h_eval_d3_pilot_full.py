...
from stage18_d3_covariance_train import SIRENCovariancePredictor
...
def main():
    ...
    # 3D Pilot Model (No Exclusion Zone)
    model = SIRENCovariancePredictor(w0=30.0, K_rank=5).to(device)
    model.load_state_dict(torch.load("siren_beamformer_d3_cov_K5_3D_no_excl.pt", map_location=device))
    model.eval()
    ...
    headings = np.linspace(0, 360, 3600, endpoint=False) # 0.1 deg
    
    for idx, el in enumerate(elevations):
        print(f"Evaluating Elevation {el}°...")
        jam_bodies_list = []
        for az in headings:
            jam_world = np.array([
        ...
        # Batch size for evaluation to avoid memory issues
        BATCH_SIZE = 500
        sinrs = []
        
        with torch.no_grad():
            for b in range(0, len(headings), BATCH_SIZE):
                inputs_t = torch.tensor(jam_bodies_np[b:b+BATCH_SIZE], device=device)
    ...
    plt.tight_layout()
    plt.savefig('diag_full_pilot_3D_no_excl.png', dpi=120)
    plt.close()
    print("Saved plot to diag_full_pilot_3D_no_excl.png")
