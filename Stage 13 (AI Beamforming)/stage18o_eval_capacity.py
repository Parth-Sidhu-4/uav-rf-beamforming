...
def main():
    ...
    # 3D Pilot Model
    model = SIRENCovariancePredictor(w0=60.0, K_rank=5, hidden_dim=512).to(device)
    model.load_state_dict(torch.load("siren_beamformer_d3_cov_K5_3D_w60_h512.pt", map_location=device))
    model.eval()
    ...
    plt.tight_layout()
    plt.savefig('diag_180deg_pilot_3D_w60_h512.png', dpi=120)
    plt.close()
