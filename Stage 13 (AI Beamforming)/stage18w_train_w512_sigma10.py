...
class ReluCovariancePredictor(nn.Module):
    def __init__(self, K_rank=5, hidden_dim=512):
        super().__init__()
        self.K = K_rank
        self.fourier = FourierEncoding(in_features=3, out_features=256, sigma=10.0)
...
def main():
    ...
    K_RANK = 5
    model = ReluCovariancePredictor(K_rank=K_RANK, hidden_dim=512).to(device)
    ...
    torch.save(model.state_dict(), 'relu_beamformer_d3_cov_K5_100k_w512.pt')
