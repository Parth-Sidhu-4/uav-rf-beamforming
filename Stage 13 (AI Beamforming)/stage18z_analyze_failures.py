...
model = ReluCovariancePredictor(K_rank=5, hidden_dim=256).to(device)
model.load_state_dict(torch.load('relu_beamformer_d3_cov_K5_100k_sigma10.pt', map_location=device))
model.eval()
...
plt.xlim(150, 210)
plt.ylim(-25, 25)
plt.title('Distribution of <15dB SINR Failures (Sigma=10.0, Width=256)')
plt.xlabel('Heading (deg)')
plt.ylabel('Elevation (deg)')
plt.grid(True, alpha=0.3)
plt.savefig('relu_failures_distribution.png', dpi=300)
print(f'Saved failure distribution to relu_failures_distribution.png')
