...
def get_hybrid_steering_vectors(headings_deg, elevations_deg, U_ai):
    ...
    """
    from stage18_interpolate_test import compute_oracle_steering_vectors, phase_aware_bilinear_interpolate
...
def main():
    ...
    from stage18_interpolate_test import phase_aware_bilinear_interpolate, compute_sinr_from_v
...
# Find where failures are
sinrs = np.array(sinrs)
below_15 = sinrs < 15.0

fail_h = headings_rear[below_15]
fail_e = elevations_rear[below_15]

# How many inside the box?
inside_box = (fail_h >= 168) & (fail_h <= 192) & (fail_e >= -27) & (fail_e <= 27)
print(f'Failures total: {len(fail_h)}, Inside box: {np.sum(inside_box)}, Outside box: {np.sum(~inside_box)}')

    print(f"Total points in rear sector: {len(sinrs)}")
    print(f"Below 15dB: {np.sum(sinrs < 15.0)} / {len(sinrs)} ({(np.sum(sinrs < 15.0)/len(sinrs))*100:.1f}%)")
